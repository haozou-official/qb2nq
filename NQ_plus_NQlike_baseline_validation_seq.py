...
args = TrainingArguments(
    f"NQ_NQlike_seq_with_alias_plot",
...
nqlike_initial_checkpoint = 500
nqlike_total_checkpoints = 123235
...
predictions_dir = './NQ_NQlike_train_seq_predictions'
...
# with open('NQ_NQlike_train_seq_NQorig_epoch1.txt', 'w') as f:
#     for item in scores:
#         f.write("%s\n" % item)

for i in range(nqlike_initial_checkpoint, nqlike_total_checkpoints, 1):
  model_path = 'NQ_NQlike_train_seq_checkpoints/epoch_1_step_'+str(i)+'_NQlike'
  flag = os.path.exists(model_path)
  if (flag==False):
    #print(model_path)
    continue
  ...
  for j in range(len(pred_answers)):
    ...
    dataset.append(data)
  # save
  with open('NQ_NQlike_train_seq_predictions/NQlike_epoch1_step_'+str(i)+'_predictions_and_references.json', 'w') as f:
      for item in dataset:
         f.write(json.dumps(item)+"\n")
...
with open('NQ_NQlike_train_seq_epoch1.txt', 'w') as f:
    for item in scores:
        f.write("%s\n" % item)
