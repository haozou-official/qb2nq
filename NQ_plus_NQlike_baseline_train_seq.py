# nqlike_train_path = './TriviaQuestion2NQ_Transform_Dataset/nqlike_with_quality_scores.json'
...
tiny_nqlike_train_tokenized_datasets = tiny_nqlike_train_data.map(prepare_train_features, batched=True, batch_size=64,
                                    remove_columns=['context', 'char_spans', 'answer',
...
nqlike_trainloader = torch.utils.data.DataLoader(tiny_nqlike_train_tokenized_datasets, batch_size=train_batch_size, collate_fn = default_data_collator, shuffle=False)
...
def train(from_pretrained_NQ):
    ...
    checkpoint_dir = "./NQ_NQlike_train_seq_checkpoints"
    ...
    nqlike_learning_rate=1e-6
    weight_decay=0.01
    nq_checkpoint_periodicity = 2000
    nqlike_checkpoint_periodicity = 500
    EPOCHS = 1
    
    opt1 = AdamW(model.parameters(), lr=nq_learning_rate, weight_decay=0.01)
    opt2 = AdamW(model.parameters(), lr=nqlike_learning_rate, weight_decay=0.01)
    
    sch1 = get_linear_schedule_with_warmup(opt1, num_warmup_steps = 0, num_training_steps = len(nq_trainloader))
    sch2 = get_linear_schedule_with_warmup(opt2, num_warmup_steps = 0, num_training_steps = len(nqlike_trainloader))
    # train
    ...
    for epoch in range(1, EPOCHS+1):
       ...
       for i,batch in enumerate(nqlike_trainloader):
          batch_loss = training_step(model, opt2, sch2, batch)
          if (i+1)%nqlike_checkpoint_periodicity == 0:
             true_labels, prediction_labels, val_loss = evaluation_step(model, valloader)
             print('NQlike Epoch ',epoch,' Batch ',i+1,' Train Loss = ',batch_loss,' Val Loss = ',val_loss)
             save_checkpoint(model, opt2, tokenizer, batch_loss, val_loss, i+1, epoch, 'NQlike')
    
       true_labels, prediction_labels, val_epoch_loss = evaluation_step(model, valloader)
       save_checkpoint(model, opt2, tokenizer, batch_loss, val_epoch_loss, i+1, epoch, 'NQlike')
...
if __name__=="__main__":
    ...
    parser.add_argument('--nqlike_train_path', type=str, default= './nqlike_with_contexts_with_quality_scores.json')
